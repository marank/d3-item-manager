...
(function () {
    ...
    function items($http, locales) {
        ...
        function load() {
            ...
            return $http.get('items/items_' + locale + '.json?', { cache: true }).then(function (result) {
                return result.data;
            });
        }
    }
...
(function () {
    ...
    function seasons(config) {
        var _all = config.getItem(keyAll, ['Season 6']);
...
(function () {
    ...
    ConfigController.$inject = ["$location", "gameModes", "columns", "seasons", "itemCategory", "locales", "backup", "restore", "exportService"];
    angular.module('d3-item-manager').controller('ConfigController', ConfigController);

    function ConfigController($location, gameModes, columns, seasons, itemCategory, locales, backup, restore, exportService) {
        var vm = this;
        vm.gameModes = gameModes;
